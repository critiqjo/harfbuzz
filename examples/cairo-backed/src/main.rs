...
const MARGIN: u32 = FONT_SIZE / 2;

fn main() {
    let mut args = env::args();
    if args.len() < 3 {
      println!("Usage: cairo-backed font-file.ttf text");
      return;
    }
    ...
    let face = lib.new_face(fontfile, 0).unwrap();
    face.set_char_size(FONT_SIZE as isize * 64, 0, FONT_SIZE * 32, 0).unwrap();
}
//  /* Create hb-ft font. */
//  hb_font_t *hb_font;
